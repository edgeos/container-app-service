...
// MergeServicesV2_1 merges a v2_1 compose file into an existing set of service configs
func MergeServicesV2_1(existingServices *ServiceConfigs, environmentLookup EnvironmentLookup, resourceLookup ResourceLookup, file string, datas RawServiceMap, options *ParseOptions) (map[string]*ServiceConfig, error) {
	if options.Validate {
		if err := validateV2_1(datas); err != nil {
			return nil, err
		}
	}

	for name, data := range datas {
		data, err := parseV2_1(resourceLookup, environmentLookup, file, data, datas, options)
		if err != nil {
	...
	if options.Validate {
		var errs []string
		for name, data := range datas {
			err := validateServiceConstraintsv2_1(data, name)
			if err != nil {
...
func parseV2_1(resourceLookup ResourceLookup, environmentLookup EnvironmentLookup, inFile string, serviceData RawService, datas RawServiceMap, options *ParseOptions) (RawService, error) {
	serviceData, err := readEnvFile(resourceLookup, inFile, serviceData)
	if err != nil {
		return nil, err
	}

	serviceData = resolveContextV2_1(inFile, serviceData)
	...
	if file == "" {
		if serviceData, ok := datas[service]; ok {
			baseService, err = parseV2_1(resourceLookup, environmentLookup, inFile, serviceData, datas, options)
		} else {
		...
		if options.Validate {
			if err := validateV2_1(baseRawServices); err != nil {
				return nil, err
			}
		}
		...
		baseService, err = parseV2_1(resourceLookup, environmentLookup, resolved, baseService, baseRawServices, options)
	}
...
func resolveContextV2_1(inFile string, serviceData RawService) RawService {
